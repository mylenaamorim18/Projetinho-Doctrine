<?php include("capsulas/capsula-inicio.php")?>

<h2 class="text-center">Cadastro de dependentes</h2>
<div class="box">

    <form class="container" action="<?php echo site_url(isset($dependente) ? 'dependente/atualizar?id='. $dependente->getId() . '&end=' . $endereco->getId() . '&tipo=dependente' : 'dependente/salvar?tipo=dependente');?>" method="POST">

    <?php if(isset($dependente)): ?>
    <input type="hidden" name="_method" value="PUT">
    <?php endif; ?>

    <div class="form-group">
        <label for="rua">Rua: </label>
        <input type="text" class="form-control" id="rua" name="rua" value="<?= isset($endereco) ? $endereco->getRua() : ''?>" placeholder="Rua">
    </div>
    <div class="form-group">
        <label for="bairro">Bairro: </label>
        <input type="text" class="form-control" id="bairro" name="bairro" value="<?= isset($endereco) ? $endereco->getBairro() : ''?>" placeholder="Bairro">
    </div>
    <div class="form-group">
        <label for="numero">Nº: </label>
        <input type="text" class="form-control" id="numero" name="numero" value="<?= isset($endereco) ? $endereco->getNumero() : ''?>" placeholder="Número da residência">
    </div>
    <div class="form-group">
        <label for="cidade">Cidade: </label>
        <input type="text" class="form-control" id="cidade" name="cidade" value="<?= isset($endereco) ? $endereco->getCidade() : ''?>" placeholder="Cidade">
    </div>

    <div class="form-group">
        <label for="estado">Estado: </label>
    <select class="form-select form-select-sm" name="estado" style="margin-top:5px; width:200px">
    <option value="<?= isset($estado_selecionado) ? $estado_selecionado->getId() : (isset($endereco) ? $estado->getId() : '')?>"><?= isset($estado_selecionado) ? $estado_selecionado->getNome() : (isset($endereco) ? $estado->getNome() : 'Selecione o estado')?></option>
        <?php
        foreach ($estados as $row) {
        ?>
            <option value="<?= $row->getId() ?>"><?= $row->getNome() ?></option>
        <?php
        }
        ?>
    </select>
    </div>

    <div class="form-group">
        <label for="nome">Nome: </label>
        <input type="text" class="form-control" id="nome" name="nome_dependente" value="<?= isset($dependente) ? $dependente->getNome() : ''?>" placeholder="Nome Sobrenome">
    </div>
    <div class="form-group">
        <label for="cpf">CPF: </label>
        <input type="text" class="form-control" id="cpf" name="cpf" value="<?= isset($dependente) ? $dependente->getCpf() : ''?>" placeholder="Ex: 000.000.000-00">
    </div>

    <div class="form-group" id="idade_depen">
        <label for="idade">Idade: </label>
        <input type="text" class="form-control" id="idade" name="idade" value="<?= isset($dependente) ? $dependente->getIdade() : ''?>" placeholder="Idade do dependente">
        
        <?php if(isset($dependente)): ?>
            <table class="table table-info">
            <thead>
                <tr>
                <th scope="col">Funcionário</th>
                <th scope="col">Excluir</th>
                </tr>
            </thead>
            <tbody>
            <?php
            foreach ($funcionarios as $row) {
            ?>
                <tr data-id="<?= $row->getId() ?>">
                <td><?= $row->getNome()?></td>
                <td class="coluna" data-fun="<?= $row->getId() ?>" data-depen="<?= $dependente->getId() ?>"><button type="button"><img class="icon" src="data:image/svg+xml;base64," width="30px"></td> <!-- mostrar que foi delatado com sucesso -->
                </tr>
            <?php
            }
            ?>
              </tbody>
            </table>  

        <?php else : ?>
        <label for="">Selecione os funcionários: </label>
        <select class="form-select form-select-sm" name="funcionarios[]" multiple aria-label=".form-select-sm" style="width:200px">
            <?php if(isset($funcionarios_selecionado)): ?>
                <?php
                foreach($funcionarios_selecionado as $row) {
                ?>
                    <option value="<?= $row->getId() ?>" selected><?= $row->getNome() ?></option>       
                <?php
                }
                ?>     
                 
            <?php else : 
            foreach ($funcionarios as $row) {
            ?>
                <option value="<?= $row->getId() ?>"><?= $row->getNome() ?></option>       
            <?php
            }
            ?>
            <?php endif; ?>
        </select>
        <?php endif; ?>
    </div>

    <?php if(isset($mensagem)): ?>
    <?= $mensagem ?>
    <?php endif; ?>
    
    <button type="submit" class="btn btt-cadastro"><?= isset($dependente) ? 'Alterar dependente' : 'Cadastrar dependente'?></button>
    </form>
</div>

<script>

$(document).on('click', '.coluna', function(event) {
    event.preventDefault();
    var dependente = $(this).data("depen");
    var funcionario = $(this).data("fun");
    var tipo = "funcionario-de-dependente";

    $.post("<?= site_url('deletarvinculado?')?>", 
    { dependente: dependente, funcionario: funcionario, tipo: tipo }, 
    function( data ) {
        $('[data-id="' + data + '"]').hide();
        notyDelete();
    }, "json");
});

</script>

<?php include("capsulas/capsula-fim.php")?>